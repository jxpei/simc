...
void affliction( player_t* p )
{
  ...
  precombat->add_action( "summon_pet", "tr= ingle_actor_batch= eport_details= esired_targets= arget_error=0.0 terations=10000 ax_time=30 arlock=TWW3_Warlock_Affliction_Hellcalle pec=afflictio evel=8 ace=maghar_or ole=spel osition=bac rofessions=alchemy=100/jewelcrafting=10 talents=CkQAAAAAAAAAAAAAAAAAAAAAAAzMzMzMjYWMwsNzMDzyAAAAmZMzyMjxyMzMLmxMDAYGLwAziRjZAZ2ALzAAAAAAAAgZYB st  alents=CkQAAAAAAAAAAAAAAAAAAAAAAAzMzMzMjYWMwsNzMDzyAAAAmZmZWMzMWmZmZDmZAAzYBGYWMaMDIzGYZGAAAAAAAAMjZ  Default consumables otion=tempered_potion_ lask=flask_of_alchemical_chaos_ ood=feast_of_the_divine_da ugmentation=crystallize emporary_enchant=main_hand:algari_mana_oil_" );
